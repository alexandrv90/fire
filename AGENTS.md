...
4. **Exceptions.** Exceptions are permitted only during initialization. Hot-path and steady-state runtime code must neither throw nor rely on exception handling; use non-throwing APIs and explicit result or status values for recoverable errors.
